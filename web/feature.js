...
async function governance(student) {
  ...
  if (form) {
    form.onsubmit = async event => {
      ...
      try {
        const data = await api('/api/petitions/submit', { method:'POST', body:JSON.stringify({ student_id:student.id, petition_type:fd.get('petition_type'), course_id:fd.get('course_id') || null, target_semester:fd.get('target_semester'), requested_credits:Number(fd.get('requested_credits') || 0), justification:fd.get('justification') }) });
        toast(data.message);
        event.target.reset();
        await governance(student);
      } catch (error) {
        toast(`Petition submission failed: ${error.message}`);
      }
    };
  }
}
...
async function reviewPetition(petitionId, decision) {
  ...
  try {
    const data = await api(`/api/petitions/${encodeURIComponent(petitionId)}/review`, { method:'POST', body:JSON.stringify({ decision, comments:`${decision.toLowerCase()} after formal constraint review.` }) });
    toast(data.message);
