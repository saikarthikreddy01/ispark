...
Architecture rule:
    LLM proposes/explains -> graph retrieves -> deterministic rules verify ->
    human faculty approves exceptions.

The supervisor dynamically routes requests to specialized agents rather than
running every agent for every question. Shared state records evidence,
verification, escalation packets, and an execution trace for the UI.
"""
...
except Exception:  # deterministic fallback remains available
    StateGraph = None
    START = END = None
...
class AcademicAdvisor:
    ...
    def _classify_node(self, state: AdvisorState) -> AdvisorState:
        ...
        return {
            "query_type": q_type,
            "agent_trace": self._trace(state, "SupervisorAgent", f"classified request as {q_type}"),
        }
    ...
    def _synthesis_node(self, state: AdvisorState) -> AdvisorState:
        ...
        return {
            ...
            "agent_trace": self._trace(state, "AdvisorSynthesisAgent", "generated source-aware natural-language advice"),
        }
    ...
    def _build_workflow(self):
        ...
        for name, fn in [
            ...
            ("conflicts", self._conflict_node),
            ("pathway", self._pathway_node),
            ("risk", self._risk_node),
            ("substitution", self._substitution_node),
            ("career", self._career_node),
            ("verify", self._verification_node),
            ("citations", self._citation_node),
            ("faculty", self._faculty_node),
        ...
        # Pathway requests get risk and career analysis before verification.
        ...
        graph.add_conditional_edges(
            "substitution",
            self._route_after_substitution,
            {"faculty": "faculty", "verify": "verify"},
        )
...
Faculty escalation packet:
{state.get('faculty_packet')}

Citation quality:
{state.get('citation_quality')}
...
Answer directly. Clearly label FORMAL BLOCK, READINESS WARNING, ADVISORY RECOMMENDATION, and FACULTY REVIEW when applicable. Cite only supplied evidence."""
    ...
    def _deterministic_response(self, state: AdvisorState) -> str:
        ...
        if state.get("faculty_packet"):
            lines.append("\n**FACULTY REVIEW REQUIRED** — the system prepared an escalation packet; it did not approve the exception.")
    ...
    def _run_without_langgraph(self, state: AdvisorState) -> AdvisorState:
        for node in [self._profile_node, self._classify_node, self._retrieve_node]:
        ...
        if route in {"prerequisite", "general"}:
            ...
            state.update(self._pathway_node(state))
